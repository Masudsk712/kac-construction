...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative z-0 min-h-screen flex items-center justify-center overflow-hidden page-hero text-slate-900 dark:text-white"
    >
      {/* VIDEO BACKGROUND */}
      <video
        autoPlay
      ...
      {/* OVERLAYS */}
      <div className="absolute inset-0 bg-white/20 dark:bg-black/65 z-[1] pointer-events-none" />
      <div className="absolute inset-0 bg-gradient-to-b from-white/30 via-white/5 to-[var(--hero-fade)] dark:from-[#020617]/55 dark:via-[#020617]/15 dark:to-[#020617] z-[2] pointer-events-none" />

      {/* GLOW EFFECTS */}
      <div className="absolute top-[-220px] left-[-220px] w-[650px] h-[650px] rounded-full bg-cyan-400/15 blur-[150px] z-[2] pointer-events-none" />
      <div className="absolute bottom-[-250px] right-[-220px] w-[650px] h-[650px] rounded-full bg-blue-500/15 blur-[160px] z-[2] pointer-events-none" />

      {/* HERO CONTENT */}
      <div className="container-premium relative z-20 flex flex-col items-center text-center pt-36 pb-28 md:pt-40">
        {/* LABEL WITH TRUST BADGES */}
        <motion.div
          variants={fadeUp}
          initial="hidden"
          animate="show"
          transition={{ duration: 0.8, ease: ease.premiumOut }}
          className="mb-8 inline-flex flex-col sm:flex-row items-center gap-4 sm:gap-5"
        >
          ...
          <div className="hidden sm:flex items-center gap-3 text-[10px] sm:text-xs uppercase tracking-[3px] text-slate-600 dark:text-white/50">
            <span className="inline-flex items-center gap-1.5">
              <ShieldCheck size={14} className="text-emerald-400" /> ISO Certified
            </span>
            <span className="w-1 h-1 rounded-full bg-slate-400" />
            <span className="inline-flex items-center gap-1.5">
              <Award size={14} className="text-amber-400" /> 15+ Years
            </span>
          </div>
        </motion.div>
        ...
        <motion.h1
          ...
          initial="hidden"
          animate="show"
          transition={{ duration: 1, delay: 0.1, ease: ease.premiumOut }}
          className="relative max-w-6xl text-[42px] sm:text-[64px] md:text-[90px] lg:text-[110px] leading-[0.9] tracking-[-2px] md:tracking-[-4px] font-black font-display"
        >
          {/* GLOW */}
      ...
      <button
        ...
        className="absolute bottom-7 left-1/2 -translate-x-1/2 z-30 cursor-pointer flex flex-col items-center text-slate-600 dark:text-white/50 hover:text-cyan-600 dark:hover:text-cyan-300 transition-all duration-300 bg-transparent border-0 p-0"
      >
        <motion.span
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1.5, duration: 0.6, ease: ease.premiumOut }}
          className="text-[10px] sm:text-[11px] tracking-[5px] mb-3"
        >
          SCROLL
        </motion.span>
        <motion.div
      ...
      {/* BOTTOM FADE */}
      <div className="absolute bottom-0 left-0 w-full h-40 bg-gradient-to-b from-transparent to-[var(--hero-fade)] z-[5] pointer-events-none" />
    </section>
  );
}